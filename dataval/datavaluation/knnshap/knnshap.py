...
class KNNShapley(DataEvaluator, ModelLessMixin):
    ...
    def train_data_values(self, *args, **kwargs):
        ...
        if isinstance(x_train, torch.Tensor):
            ...
            x_train_tensor = self.subset_to_tensor(x_train)
            x_train_view = x_train_tensor.view(n, -1)
            
        if isinstance(x_valid, torch.Tensor):
            x_valid_view = x_valid.view(m, -1)
        else:
            x_valid_tensor = self.subset_to_tensor(x_valid)
            x_valid_view = x_valid_tensor.view(m, -1)
    ...
    # def subset_to_tensor(self, subset):
    #     data_list = [subset[i][0] for i in range(len(subset))]
