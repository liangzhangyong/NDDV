...
from dataval.datavaluation.ndsv.meta_inter import MetaInter
...
class NDSV(DataEvaluator, ModelMixin):
    ...
    def train_data_values(self, *args, num_workers: int = 0, **kwargs):
        ...
        if self.base_model.lower() == "classifiermlp":
            self.mfg_net = ClassifierMFG(self.input_dim, self.num_classes, batch_size, hidden_dim=self.hidden_dim, Ntime=self.Ntime, totalT=self.totalT, sigma=self.sigma,interact=self.interact, interpret=self.interpret, kernel=self.kernel, device=self.device).to(device=self.device)
        ...
        # Solver
        if self.interpret:
            print('Interpretation NDDV model')
            self.meta_net = MetaInter(hidden_size=self.meta_hidden_size, kernel=self.kernel).to(device=self.device)
            optimizer = torch.optim.Adam(self.mfg_net.parameters(), lr=self.lr, weight_decay=self.weight_decay)
            meta_optimizer = torch.optim.Adam(self.meta_net.parameters(), lr=self.meta_lr, weight_decay=self.meta_weight_decay)
        else:
            self.meta_net = Meta(self.meta_hidden_size, self.meta_num_layers).to(device=self.device)
            optimizer = torch.optim.AdamW(self.mfg_net.parameters(), lr=self.lr, weight_decay=self.weight_decay)
            meta_optimizer = torch.optim.AdamW(self.meta_net.parameters(), lr=self.meta_lr, weight_decay=self.meta_weight_decay)
        ...
        self.mfg_net.train()
        self.data_loss = torch.zeros(0, 1, device=self.device)
        for epoch in tqdm(range(self.mfg_epochs)):
            ...
            for x_batch, y_batch in train_loader:
                ...
                if (iteration + 1) % self.meta_interval == 0:
                    ...
                    if self.re_weight:
                        ...
                        pseudo_loss_vector_weight = pseudo_loss_vector_reshape
                ...
                # extract data loss
                if epoch == self.mfg_epochs - 1:
                    self.data_loss = torch.cat([self.data_loss, loss_vector_weight.detach()])
                
                # extract state trajectory
                if epoch in [5, 10, 30, 49]:
                    x_epoch = [torch.mean(out, dim=1, keepdim=True) for out in outputs_x]
                    y_epoch = [torch.mean(out, dim=1, keepdim=True) for out in outputs_y][::-1]
                    x_traj = torch.stack(x_epoch[:self.Ntime+1], dim=0).detach().cpu().numpy()
                    x_trajectories.append(x_traj)
                    y_traj= torch.stack(y_epoch[:self.Ntime+1], dim=0).detach().cpu().numpy()
            ...
            if epoch in [5, 10, 30, 49]:
                self.x_trajectories.append(np.concatenate(x_trajectories, 1))
                self.y_trajectories.append(np.concatenate(y_trajectories, 1)[::-1])
        ...
        self.mu = mu
        # self.data_loss = self.data_loss.squeeze()
    ...
    def data_values_trajectory(self):
        ...
        for x_batch, y_batch in data_loader:
            ...
            dvt.mf = False
            dvt.inputs = x_batch
            outputs_x, outputs_w, _ = self.mfg_net(dvt)
            terminal_loss = self.criterion(outputs_x[-1], y_batch)
            # dvt.mode = 'HamGraFlow'
            # dvt.xMat = outputs_x
            # dvt.wMat = outputs_w
            # dvt.loss = terminal_loss
            # batch_data_values = self.mfg_net(dvt)
    ...
    def data_costate_trajectory(self):
        ...
        for x_batch, y_batch in data_loader:
            ...
            dcst.mf = False
            dcst.inputs = x_batch
            outputs_x, outputs_w, _ = self.mfg_net(dcst)
            loss_vector = self.criterion(outputs_x[-1], y_batch, reduction='none')
            if loss_vector.dim() == 2:
                loss_vector_reshape = torch.reshape(loss_vector[:,0], (-1, 1)) 
            elif loss_vector.dim() == 1:
                loss_vector_reshape = torch.reshape(loss_vector, (-1, 1))
            dcst.mode = 'backwardYZ'
            dcst.xMat = outputs_x
            dcst.wMat = outputs_w
            dcst.loss = loss_vector_reshape
            outputs_y, _ = self.mfg_net(dcst)
            outputs = [torch.mean(out, dim=1, keepdim=True) for out in outputs_y][::-1]
            traj = torch.stack(outputs[:self.Ntime+1], dim=0).detach().cpu().numpy()
            trajectories.append(traj)
            classes.extend(y_batch.cpu().numpy())
            
        trajectories = np.concatenate(trajectories, 1)
        
        return time_span, trajectories[::-1]
    ...
    def save_x_y_state(self):
        ...
        return self.x_trajectories, self.y_trajectories