"""Experiments to test :py:class:`~dataval.dataval.api.DataEvaluator`.
...
from dataval.datavaluation import DataEvaluator
from dataval.experiment.util import f1_score, oned_twonn_clustering, oned_twonn_clustering_filter, EOp_score, EOdds_score
from dataval.metrics import Metrics
from dataval.model import Model
from dataval.util import get_name
...
def noisy_detection(
    ...
    for i in range(len(label_indices)):
        label_noisy_indices.append(np.intersect1d(noisy_train_indices, label_indices[i]))
    
    if filter:
        unvaluable, _ = oned_twonn_clustering_filter(data_values.flatten())
    else:
    ...
    EOp_kmeans_label = EOp_score(unvaluable, noisy_train_indices, label_noisy_indices, len(data_values))
    EOdds_kmeans_label = EOdds_score(unvaluable, noisy_train_indices, label_noisy_indices, len(data_values))

    # return {"kmeans_f1": f1_kmeans_label, "kmeans_EOp": EOp_kmeans_label, "kmeans_EOdds": EOdds_kmeans_label}
    return {"kmeans_f1": f1_kmeans_label}


def dynamic_data_state(
    evaluator: DataEvaluator,
    fetcher: Optional[DataFetcher] = None,
    indices: Optional[list[int]] = None,
    ):
    time, traj = evaluator.data_state_traject
    
    return time, traj


def dynamic_data_costate(
    evaluator: DataEvaluator,
    fetcher: Optional[DataFetcher] = None,
    indices: Optional[list[int]] = None,
    ):
    time, traj = evaluator.data_costate_traject
    
    return time, traj


def dynamic_data_values(
    evaluator: DataEvaluator,
    fetcher: Optional[DataFetcher] = None,
    indices: Optional[list[int]] = None,
    ):
    time, traj = evaluator.data_values_traject
...
def extract_loss(
    ...
    train_loss, meta_loss = evaluator.ext_train_meta_loss
    
    return train_loss, meta_loss


def extract_data_loss(
    evaluator: DataEvaluator,
    fetcher: Optional[DataFetcher] = None,
    indices: Optional[list[int]] = None,
    ):
    data_loss = evaluator.ext_data_loss
...
def extract_state(
    ...
    x_state, y_state = evaluator.ext_x_y_state
...
def remove_high_value(
    ...
    for bin_index in range(0, num_points, num_period):
        ...
        iy_hat_valid = valuable_model.predict(x_test).to("cpu")
        valuable_score = metric(y_test, iy_hat_valid)
        valuable_list.append(valuable_score)

    x_axis = [i / num_bins for i in range(num_bins)]

    eval_results = {
        f"remove_most_influential_first_{get_name(metric)}": valuable_list,
        "axis": x_axis,
    }
    ...
    if plot is not None:
        # Prediction performances after removing high or low values
        plot.plot(x_axis, valuable_list[:num_bins], "x-")

        plot.set_xlabel("Fraction Removed")
        plot.set_ylabel(get_name(metric))
        plot.legend(["Removing high value data"])
...
def remove_low_value(
    ...
    if plot is not None:
        ...
        plot.set_xlabel("Fraction Removed")
        plot.set_ylabel(get_name(metric))
        plot.legend(["Removing low value data"])
...
def add_low_high(
    ...
    # Start with an empty model and add points
    for bin_index in range(num_period, num_points + num_period, num_period):
        # Adding least valuable samples first
        most_valuable_indices = sorted_value_list[:num_period+bin_index]
        ...
        # Adding most valuable samples first
        least_valuable_indices = sorted_value_list[:num_points+bin_index]
    ...
    x_axis = [i / num_bins for i in range(num_bins)][::-1]

    eval_results = {
        f"add_least_influential_first_{get_name(metric)}": valuable_list,
        f"add_most_influential_first_{get_name(metric)}": unvaluable_list,
        "axis": x_axis,
    }
    ...
    if plot is not None:
        ...
        plot.set_ylabel(get_name(metric))
        plot.legend(["Adding low value data", "Adding high value data"])
...
def add_low_value(
    ...
    for bin_index in range(num_period, num_points + num_period, num_period):
        # Adding least valuable samples first
        least_valuable_indices = sorted_value_list[:num_period+bin_index]
    ...
    if plot is not None:
        ...
        plot.set_xlabel("Fraction Added")
        plot.set_ylabel(get_name(metric))
        plot.legend(["Adding low value data"])
...
def add_high_value(
    ...
    for bin_index in range(num_period, num_points + num_period, num_period):
        # Adding most valuable samples first
        most_valuable_indices = sorted_value_list[:num_points+bin_index]
    ...
    eval_results = {
        f"add_most_influential_first_{get_name(metric)}": valuable_list,
        "axis": x_axis,
    }
    ...
    if plot is not None:
        ...
        plot.set_xlabel("Fraction Added")
        plot.set_ylabel(get_name(metric))
        plot.legend(["Adding high value data"])
