...
class ClassifierMFG(TorchClassMixinMFG, TorchPredictMixinMFG, TorchGradMixinMFG):
    ...
    def __init__(
        ...
        # self.mfg = nn.ModuleList([ControlNN(self.hidden,self.grid_min,self.grid_max,self.num_grids,self.spline_scale) for _ in range(self.Ntime)])
...
class ControlKAN(nn.Module):
    def __init__(self, num_hiddens: list[int], grid_min: float, grid_max: float, num_grids: int, spline_scale: float):
        super(ControlKAN, self).__init__()
        self.layers = [GRBFKANLayer(num_hiddens[i-1], num_hiddens[i], grid_min, grid_max, num_grids, spline_scale) for i in range(1, len(num_hiddens))]
        self.layers = nn.Sequential(*self.layers)
