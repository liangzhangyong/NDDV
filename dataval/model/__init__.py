...
.. currentmodule:: dataval.model

:py:class:`Model` is an ABC used to take an existing model and make it compatible with
the :py:class:`~dataval.dataval.DataEvaluator` and other related objects.
...
from dataval.model.bert import BertClassifier
from dataval.model.grad import GradientModel, TorchGradMixin
from dataval.model.lenet import LeNet
from dataval.model.logistic_regression import LogisticRegression
from dataval.model.mlp import ClassifierMLP, RegressionMLP
from dataval.model.mfg import ClassifierMFG, RegressionMFG, LogisticRegressionMFG
from dataval.model.neural_ode import ConvNeuralODE
...
def ModelFactory(  # noqa: C901 model factory tries to match name with long if-else
    ...
    **kwargs,
    ...
    elif model_name == "logisticregressionmfg":
        return LogisticRegressionMFG(*covar_dim, *label_dim, batch_size=batch_size, *args, **kwargs).to(device)
    elif model_name == "classifiermlp":
        return ClassifierMLP(*covar_dim, *label_dim, *args, **kwargs).to(device)
    elif model_name == "classifiermfg":
        return ClassifierMFG(*covar_dim, *label_dim, batch_size=batch_size, *args, **kwargs).to(device)
    elif model_name == "regressionmlp":
        return RegressionMLP(*covar_dim, *label_dim, *args, **kwargs).to(device)
    elif model_name == "regressionmfg":
        return RegressionMFG(*covar_dim, *label_dim, batch_size=batch_size, *args, **kwargs).to(device)
