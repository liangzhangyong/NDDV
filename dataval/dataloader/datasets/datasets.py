...
from dataval.dataloader.register import Register, cache
...
@Register("vehicle_sensIT", one_hot=True)
def download_vehicle_sensIT():
    """Categorical data set registered as ``"vehicle_sensIT"``."""
    return load_openml(data_id=357)


@Register("webdata_wXa", one_hot=True)
def download_webdata_wXa():
    """Categorical data set registered as ``"webdata_wXa"``."""
    return load_openml(data_id=350)
