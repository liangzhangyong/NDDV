...
class DataFetcher:
    ...
    def _presplit_data(self, x_train, x_valid, x_test, x_meta, y_train, y_valid, y_test, y_meta):
        ...
        if not len(x_meta) == len(y_meta):
            raise ValueError("Metaing Covariates and Labels must be of same length.")

        if not (x_train[0].shape == x_valid[0].shape == x_test[0].shape == x_meta[0].shape):
            raise ValueError("Covariates must be of same shape.")
        if not (y_train[0].shape == y_valid[0].shape == y_test[0].shape == y_meta[0].shape):
            raise ValueError("Labels must be of same shape.")

        self.x_train, self.x_valid, self.x_test, self.x_meta = x_train, x_valid, x_test, x_meta
        self.y_train, self.y_valid, self.y_test, self.y_meta = y_train, y_valid, y_test, y_meta

        tr, val, test, meta = len(self.x_train), len(self.x_valid), len(self.x_test), len(self.x_meta)
        ...
        self.meta_indices = np.fromiter(range(tr + val+test, tr + val + test+meta), dtype=int)
    ...
    def setup(
        ...
        split_types = (type(train_count), type(valid_count), type(test_count), type(meta_count))
        if split_types == (int, int, int, int):
            return (
                cls(dataset_name, cache_dir, force_download, random_state)
                .split_dataset_by_count(train_count, valid_count, test_count, meta_count)
                .noisify(add_noise, **noise_kwargs)
            )
        elif split_types == (float, float, float, float):
            return (
                cls(dataset_name, cache_dir, force_download, random_state)
                .split_dataset_by_prop(train_count, valid_count, test_count, meta_count)
                .noisify(add_noise, **noise_kwargs)
            )
        else:
            raise ValueError(
                f"Expected split types to all of int or float but got "
                f"{type(train_count)=}|{type(valid_count)}|{type(test_count)=}|{type(meta_count)=}"
            )
    ...
    def from_data_splits(
        ...
        y_test: np.ndarray,
        x_meta: Union[Dataset, np.ndarray],
        y_meta: np.ndarray,
        one_hot: bool,
        random_state: Optional[RandomState] = None,
    ):
    ...
    def datapoints(self):
        ...
        """
        x_trn, x_val, x_test, x_meta = self.x_train, self.x_valid, self.x_test, self.x_meta
        ...
        if not isinstance(self.x_train, Dataset) and not isinstance(
            ...
            # x_meta = torch.tensor(x_meta, dtype=torch.float).view(-1, *self.covar_dim)
        ...
        # y_meta = torch.tensor(self.y_meta, dtype=torch.float).view(-1, *self.label_dim)

        return x_trn, y_trn, x_val, y_val, x_test, y_test
    
    @property
    def metadata(self):
        """Return metadata of the data set.
        """
        x_meta, y_meta = self.x_meta, self.y_meta
        if not isinstance(self.x_meta, Dataset) and not isinstance(
            self.x_meta, torch.Tensor
        ):  # Turns arrays -> cpu tensors
            x_meta = torch.tensor(x_meta, dtype=torch.float).view(-1, *self.covar_dim)
        y_meta = torch.tensor(self.y_meta, dtype=torch.float).view(-1, *self.label_dim)
    ...
    def split_dataset_by_prop(
        ...
        train_count, valid_count, test_count, meta_prop = (
            round(self.num_points * p) for p in (train_prop, valid_prop, test_prop, meta_prop)
        )
        return self.split_dataset_by_count(train_count, valid_count, test_count, meta_prop)
    ...
    def split_dataset_by_count(
        ...
        if sum((train_count, valid_count, test_count, meta_count)) > self.num_points:
            raise ValueError(
                f"Split totals must be < {self.num_points=} and of the same type: ")
            
        # Meta data
        num_classes = len(np.unique(self.labels))
        num_meta_per_class = int(meta_count / num_classes)
        for class_index in range(num_classes):
            class_indices = np.where(self.labels[:, 0] == class_index)[0]
            np.random.shuffle(class_indices)
            meta_indices = class_indices[:num_meta_per_class]
            data_indices = class_indices[num_meta_per_class:]
            if class_index == 0:
                meta_indices_all = meta_indices
                data_indices_all = data_indices
            else:
                meta_indices_all = np.concatenate((meta_indices_all, meta_indices))
                data_indices_all = np.concatenate((data_indices_all, data_indices))
                
        self.meta_indices = meta_indices_all
        self.data_indices = data_indices_all
        
        if isinstance(self.covar, Dataset):
            self.x_meta = Subset(self.covar, self.meta_indices)
            self.covar = Subset(self.covar, self.data_indices)
        else:
            self.x_meta = self.covar[self.meta_indices]
            self.covar = self.covar[self.data_indices]
        
        self.y_meta = self.labels[self.meta_indices]
        self.labels = self.labels[self.data_indices]
        
        self.num_points_new = len(self.data_indices)
        
        # Split the data set into train/valid/test
        sp = list(accumulate((train_count, valid_count, test_count)))

        # Extra underscore to unpack any remainders
        idx = self.random_state.permutation(self.num_points_new)
    ...
    def split_dataset_by_indices(
        ...
        meta_indices: Optional[Sequence[int]] = None,
    ):
        ...
        meta_indices = [] if meta_indices is None else meta_indices

        idx = chain(train_indices, valid_indices, test_indices, meta_indices)
        seen = set()
        for index in idx:
        ...
        self.meta_indices = np.array(meta_indices, dtype=int)
    ...
    def noisify(
        ...
        **noise_kwargs,
        ...
        if isinstance(add_noise, str):
            from dataval.dataloader.noisify import NoiseFunc
