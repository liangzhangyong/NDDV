...
from dataval.dataloader.util import IndexTransformDataset
from dataval.util import FuncEnum
...
def add_gauss_noise(
    fetcher: DataFetcher, noise_rate: float = 0.1, mu: float = 1.0, sigma: float = 0.1
...
def add_uniform_noise(
    fetcher: DataFetcher, noise_rate: float = 0.2, low: float = -1.0, high: float = 1.0
) -> dict[str, Union[Dataset, np.ndarray]]:
    """Add uniform noise to covariates.
    ...
    noise_rate : float
        Proportion of labels to add noise to
    low : float, optional
        Lower bound of the uniform distribution from which noise is generated, by default -1
    high : float, optional
        Upper bound of the uniform distribution from which noise is generated, by default 1
    ...
    dict[str, np.ndarray]
        ...
        - **"x_train"** -- Updated training covariates with added uniform noise
        - **"noisy_train_indices"** -- Indices of training data set with mixed labels
    """
    rs = check_random_state(fetcher.random_state)
    ...
    noisy_train_idx = rs.choice(num_train, round(num_train * noise_rate), replace=False)
    noisy_valid_idx = rs.choice(num_valid, round(num_valid * noise_rate), replace=False)
    noise_train = rs.uniform(low, high, size=(len(noisy_train_idx), *feature_dim))
    noise_valid = rs.uniform(low, high, size=(len(noisy_valid_idx), *feature_dim))
...
class NoiseFunc(FuncEnum):
    ...
    ADD_UNIFORM_NOIDE = FuncEnum.wrap(add_uniform_noise)
