...
from dataval.dataloader import NoiseFunc, Register
from dataval.dataval import DataEvaluator
from dataval.experiment import ExperimentMediator
from dataval.experiment import exper_methods as em
from dataval.metrics import Metrics
from dataval.model import Model
from dataval.util import StrEnum
...
def run(
    ...
    """CLI to run an dataval job form input parameters
    ...
    model : ModelsEnum, str
        Name of the model, should match class name, see py:function`~dataval.model.ModelFactory`
    add_noise : NoiseFunc, str, optional
        Method of adding artificial noise, must match names in py:class`~dataval.dataloader.NoiseFunc`
    metric : Metrics, str, optional
        Method of evaluating the prediction model, must match names in py:class`~dataval.metrics.Metrics`
